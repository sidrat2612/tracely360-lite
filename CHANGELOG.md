...
- Fix: AST cache written to `src/tracely360-out/cache/` instead of project root — `extract()` now called with explicit `cache_root` (#429)
- Fix: `.mdx` files silently skipped during detection — added to `DOC_EXTENSIONS` (#428)

## 0.4.21 (2026-04-17)

- Fix: `tracely360 cluster-only` crashed with `KeyError: 'total_files'` — cluster-only skips detection; report now handles missing stats (#422)
- Fix: `/tracely360 --update` dropped existing graph nodes — merged result now serialized before Step 4 (#423)

## 0.4.20 (2026-04-17)

- Fix: JS/MJS `imports_from` edges dropped for `../subdir/file.mjs` style imports — fixed with `os.path.normpath` (#414)
- Fix: `tracely360 update .` and `cluster-only` now generate `graph.html` alongside other outputs (#418)
...
- Fix: `tracely360 hook install` respects `core.hooksPath` git config (#401)
...
- Docs: team workflow section — committing output, `.tracely360ignore` usage (#369)

## 0.4.16 (2026-04-16)

- Fix: `tracely360 watch` crashed with NameError — missing `import sys` (#386, #394)
- Fix: `.mjs` files detected but produced 0 nodes — added to AST dispatch table (#387)
- Fix: `llm.py` excluded from published wheel (#391)

## 0.4.15 (2026-04-15)

- Feat: VS Code Copilot Chat support — `tracely360 vscode install` (#206)
...
- Fix: macOS `tracely360 watch` uses `PollingObserver` by default (#373)
...
- Fix: `tracely360 install --platform opencode` also installs plugin (#356)
...
- Fix: `tracely360 update` exits with code 1 on failure (#287)
- Fix: templates use `tracely360 update .` instead of hardcoded `python3 -c` (#287)
...
- Fix: rebuild rule uses `tracely360 update .` instead of hardcoded `python3 -c` (#324)
- Fix: `tracely360 query` handles `label: null` nodes (#323)

## 0.4.12 (2026-04-13)

- Add: Kiro IDE/CLI support — `tracely360 kiro install` (#319, #321)
- Fix: cache `file_hash()` uses relative paths — portable across machines (#311)

## 0.4.11 (2026-04-13)

- Fix: `tracely360 query` on MultiGraph graphs (#305)
- Fix: `tracely360 query` handles null `source_file` (#307)
- Fix: MCP server derives base from absolute graph path (#309)
- Fix: `.tracely360ignore` patterns from parent directories (#303)

## 0.4.10 (2026-04-13)

- Fix: `tracely360 install --platform cursor` crash (#281)
...
- Fix: `tracely360 claw install` writes to `.openclaw/` (#208)
...
- Fix: `collect_files()` respects `.tracely360ignore` (#188)
...
- Add: `tracely360-out/transcripts/` cache
- Requires: `pip install 'tracely360[video]'`
...
- Fix: `tracely360 install --platform gemini` copies skill to Gemini CLI (#174)
...
- Fix: `.tracely360ignore` walks parent directories (#168)
...
- Fix: `tracely360 codex install` recovery on re-run (#153)
...
- Fix: `--watch` mode respects `.tracely360ignore` (#120)
...
- Fix: `.tracely360_python` preserved across runs (#92)
...
- Fix: skill intermediate files moved to `tracely360-out/` (#81)
...
- Add: `tracely360 query` CLI command
...
- Add: `tracely360 install --platform <codex|opencode|claw>`
...
- Add: `tracely360 claude install/uninstall`
- Add: `tracely360 hook install/uninstall/status`
- Add: `tracely360 benchmark` CLI command
...
- Fix: stale skill version warning persists after running `tracely360 install` when multiple platforms were previously installed — `tracely360 install` now refreshes `.tracely360_version` in all other known skill directories so the warning clears across the board (#178)
...
- Fix: AST cache written to `src/tracely360-out/cache/` instead of project root when all code files share a common prefix like `src/` — `extract()` now called with explicit `cache_root=watch_path` in `_rebuild_code` and `cache_root=Path('.')` in the Codex skill AST step (#429)
...
- Fix: `tracely360 cluster-only` crashed with `KeyError: 'total_files'` in `report.py` — cluster-only skips detection so the stats dict was empty; now passes a `warning` key so the report skips the file-stats section (#422)
- Fix: `/tracely360 --update` dropped all existing graph nodes — the merge block built a correct in-memory `G_existing` but never wrote it back to `.tracely360_extract.json`, so Step 4 rebuilt from the new-extraction-only file; merged result is now serialized back before Step 4 runs (#423)
...
- Fix: `tracely360 update .` and `tracely360 cluster-only` now generate `graph.html` alongside `graph.json` and `GRAPH_REPORT.md` — previously only the skill generated the interactive HTML (#418)
...
- Fix: Windows `tracely360-out/tracely360-out` nesting bug — `cache_dir` and `_rebuild_code` in watch.py now call `.resolve()` on the root path, preventing a nested output directory when tracely360 is run from a subdirectory (#410)
- Fix: `tracely360 hook install` now respects `core.hooksPath` git config (used by Husky and similar tools) — hooks are written to the configured path instead of always `.git/hooks` (#401)
...
- Docs: added team workflow section — committing `tracely360-out/`, `.tracely360ignore` usage, and recommended `.gitignore` additions (#369)

## 0.4.16 (2026-04-16)

- Fix: tracely360 watch crashed on all platforms with NameError because import sys was missing from watch.py (#386, #394)
...
- Feat: VS Code Copilot Chat support — `tracely360 vscode install` installs a Python-only skill (works on Windows PowerShell) and writes `.github/copilot-instructions.md` for always-on graph context (#206)
...
- Fix: macOS `tracely360 watch` now uses `PollingObserver` by default to avoid missed events with FSEvents (#373)
...
- Fix: `--wiki` flag now runs `to_wiki()` as Step 6b in the skill pipeline before the cleanup step — community labels are available and the wiki is written to `tracely360-out/wiki/` (#229, #354)
- Fix: `tracely360 install --platform opencode` now also installs the `.opencode/plugins/tracely360.js` plugin, matching what `tracely360 opencode install` does (#356)
- Fix: `extract()` accepts explicit `cache_root` parameter so subdirectory runs no longer write cache to `<subdir>/tracely360-out/cache/` (#350)
- Fix: `os.replace` in cache writer falls back to `shutil.copy2` on `PermissionError` (Windows WinError 5) (#287)
- Fix: `tracely360 update` exits with code 1 on rebuild failure instead of silently returning (#287)
- Fix: `CLAUDE.md`, Cursor, and Antigravity templates now use `tracely360 update .` instead of hardcoded `python3 -c` invocation (#287)
- Fix: `skill-kiro.md` added to `pyproject.toml` package-data — `tracely360 kiro install` was failing on fresh pip installs (#352)
...
- Fix: AGENTS.md and GEMINI.md rebuild rule now uses `tracely360 update .` instead of hardcoded `python3 -c "..."` — correct Python is resolved through the tracely360 binary, no more interpreter mismatches in Nix/pipx/uv environments (#324)
- Fix: `tracely360 query` and `tracely360 explain` no longer crash with `AttributeError` when a node has `label: null` — all `.get("label", "")` calls guarded with `or ""` to handle explicit null values (#323)

## 0.4.12 (2026-04-13)

- Add: Kiro IDE/CLI support — `tracely360 kiro install` writes `.kiro/skills/tracely360/SKILL.md` (invoked via `/tracely360`) and `.kiro/steering/tracely360.md` (`inclusion: always` — always-on context before every conversation) (#319, #321)
...
- Fix: `tracely360 query` no longer crashes with `ValueError` on MultiGraph graphs — `G.edges[u, v]` replaced with `G[u][v]` + MultiGraph guard (#305)
- Fix: `tracely360 query` no longer crashes with `AttributeError: 'NoneType' has no attribute 'lower'` when a node has a null `source_file` (#307)
- Fix: MCP server launched from a different directory now correctly derives the `tracely360-out` base from the absolute path provided, instead of CWD (#309)
- Fix: `.tracely360ignore` patterns from a parent directory now fire correctly when tracely360 is run on a subfolder — patterns are matched against paths relative to both the scan root and the `.tracely360ignore`'s anchor directory (#303)

## 0.4.10 (2026-04-13)

- Fix: `tracely360 install --platform cursor` no longer crashes — passes `Path(".")` to `_cursor_install` (#281)
...
- Add: Hermes Agent platform support — `tracely360 hermes install` writes skill to `~/.hermes/skills/tracely360/SKILL.md` and AGENTS.md (#251)
...
- Fix: `tracely360 claw install` now writes to `.openclaw/` (correct OpenClaw directory) instead of `.claw/` (#208)
- Add: Blade template support — `@include`, `<livewire:>` components, and `wire:click` bindings extracted from `.blade.php` files (#242)
- Docs: WSL/Linux MCP setup note — package name is `tracely360`, use `.venv/bin/python3` in `.mcp.json` (#250)

## 0.4.6 (2026-04-12)

- Add: Google Antigravity support — `tracely360 antigravity install` writes `.agent/rules/tracely360.md` (always-on rules) and `.agent/workflows/tracely360.md` (`/tracely360` slash command) (#203, #199, #53)
...
- Fix: Windows hooks now fall back to `python` if `python3` is not found; exits cleanly if neither has tracely360 installed (#244)
...
- Fix: `collect_files()` in `extract.py` now respects `.tracely360ignore` — previously ignored patterns, causing thousands of unwanted files (e.g. `node_modules/`) to be scanned (#188)
...
- Branch: v4 — video and audio corpus support
- Add: drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and tracely360 transcribes them locally with faster-whisper before extraction
- Add: YouTube and URL download via yt-dlp — `/tracely360 add https://youtube.com/...` downloads audio-only and feeds it through the same Whisper pipeline
...
- Add: `tracely360-out/transcripts/` cache — transcripts cached by filename; YouTube URLs cached by hash so re-runs skip already-transcribed files
- Requires: `pip install 'tracely360[video]'` for faster-whisper and yt-dlp

## 0.3.29 (2026-04-10)

- Add: video and audio corpus support — drop `.mp4`, `.mp3`, `.wav`, `.mov`, `.webm`, `.m4a`, `.ogg`, `.mkv`, `.avi`, `.m4v` files into any corpus and tracely360 transcribes them with faster-whisper before extraction
- Add: YouTube and URL video download — pass a YouTube link (or any video URL) to `/tracely360 add <url>` and yt-dlp downloads audio-only, which is then transcribed and added to the corpus automatically
...
- Add: `tracely360-out/transcripts/` cache — transcripts are cached by filename so re-runs skip already-transcribed files; URLs cached by hash
- Requires: `pip install 'tracely360[video]'` for faster-whisper + yt-dlp
...
- Fix: tracely360 install --platform gemini now also copies the skill file to ~/.gemini/skills/tracely360/SKILL.md so the /tracely360 trigger works in Gemini CLI (#174)
...
- Fix: `tracely360 install --platform gemini` now routes to `gemini_install()` instead of erroring — `gemini` was missing from `_PLATFORM_CONFIG` (#171)
- Fix: `tracely360 install --platform cursor` now routes to `_cursor_install()` the same way (#171)
...
- Fix: `_load_tracely360ignore` now walks parent directories up to the `.git` boundary, matching `.gitignore` discovery behavior — subdirectory scans now inherit root ignore patterns (#168)
- Add: Aider platform support — `tracely360 install --platform aider` copies skill to `~/.aider/tracely360/SKILL.md`; `tracely360 aider install/uninstall` writes AGENTS.md rules (#74)
- Add: GitHub Copilot CLI platform support — `tracely360 install --platform copilot` copies skill to `~/.copilot/skills/tracely360/SKILL.md`; `tracely360 copilot install/uninstall` for skill management (#134)
...
- Fix: `tracely360 codex install` (and opencode) no longer exits early when `AGENTS.md` already has the tracely360 section — partial installs with a missing `.codex/hooks.json` can now recover on re-run (#153)

## 0.3.23 (2026-04-09)

- Add: Gemini CLI support — `tracely360 gemini install` writes a `GEMINI.md` section and a `BeforeTool` hook in `.gemini/settings.json` that fires before file-read tool calls (#105)
...
- Add: Cursor support — `tracely360 cursor install` writes `.cursor/rules/tracely360.mdc` with `alwaysApply: true` so the graph context is always included; `tracely360 cursor uninstall` removes it (#137)
...
- Fix: README clarifies `tracely360` is the only official PyPI package — other `tracely360*` packages are not affiliated (#129)
...
- Add: OpenCode `tool.execute.before` plugin — `tracely360 opencode install` now writes `.opencode/plugins/tracely360.js` and registers it in `opencode.json`, firing the graph reminder before bash calls (equivalent to Claude Code's PreToolUse hook) (#71)
...
- Fix: `--watch` mode now respects `.tracely360ignore` — `_rebuild_code` was calling `collect_files()` directly instead of `detect()`, bypassing ignore patterns (#120)
...
- Refactor: `save_query_result` inline Python blocks in all 6 skill files replaced with `tracely360 save-result` CLI command — shorter, maintainable, less tokens for LLM (#114)
- Add: `tracely360 save-result` CLI subcommand — saves Q&A results to memory dir without inline Python
...
- Fix: `mkdir -p tracely360-out` now runs before writing `.tracely360_python` in `skill.md` — prevents write failure on first run; `.tracely360_python` no longer deleted in Step 9 cleanup across all skill files so follow-up commands keep their interpreter (#93)
...
- Fix: `tracely360 query`, `serve`, and `benchmark` now work on NetworkX < 3.4 — version-safe shim for `node_link_graph()` at all call sites (#95)
- Fix: `.jsx` files now detected and extracted via the JS extractor — added to `CODE_EXTENSIONS` and `_DISPATCH` (#94)
- Fix: `.tracely360_python` no longer deleted in Step 9 cleanup across all 6 skill files — pipx users no longer hit `ModuleNotFoundError` on follow-up commands (#92)

## 0.3.15 (2026-04-08)

- Feat: Trae and Trae CN platform support (`tracely360 install --platform trae` / `trae-cn`)
...
- Fix: pipx installs now detected correctly in git hooks (reads shebang from tracely360 binary)
...
- Fix: `tracely360 codex install` now also writes a PreToolUse hook to `.codex/hooks.json` so the graph reminder fires before every Bash tool call (#86)
...
- Fix: Skill intermediate files (`.tracely360_*.json`) now live in `tracely360-out/` instead of project root, preventing git pollution (#81)
...
- Fix: Windows UnicodeEncodeError during `tracely360 install` — replaced arrow character with `->` in all print statements (#47)
...
- Docs: Codex uses `$tracely360 .` not `/tracely360 .` (#36)
...
- Add: `tracely360 query "<question>"` CLI command — BFS/DFS traversal of `graph.json` without needing Claude Code skill (`--dfs`, `--budget N`, `--graph <path>` flags)
...
- Add: `--obsidian-dir <path>` flag — write Obsidian vault to a custom directory instead of `tracely360-out/obsidian`
...
- Add: `tracely360 install --platform <codex|opencode|claw>` routes skill to correct config directory
- Add: `tracely360 codex install` / `opencode install` / `claw install` — writes AGENTS.md for always-on graph-first behaviour
- Add: `tracely360 claude uninstall` / `codex uninstall` / `opencode uninstall` / `claw uninstall`
- Add: Apache 2.0 license
...
- Add: `tracely360 claude install` — writes tracely360 section to local CLAUDE.md + PreToolUse hook in `.claude/settings.json`
- Add: `tracely360 claude uninstall` — removes section and hook
- Add: `tracely360 hook install` — installs post-commit and post-checkout git hooks (platform-agnostic)
- Add: `tracely360 hook uninstall` / `hook status`
- Add: `tracely360 benchmark` CLI command
- Fix: node deduplication documented at all three layers

## 0.1.8 (2026-04-05)

- Fix: follow-up questions now check for wiki first (tracely360-out/wiki/index.md) before falling back to graph.json
- Fix: --update now auto-regenerates wiki if tracely360-out/wiki/ exists
- Fix: community articles show truncation notice ("... and N more nodes") when > 25 nodes
- UX: pipeline completion message now lists all available flags and commands so users know what tracely360 can do
...
- Add: `tracely360/wiki.py` module with `to_wiki()` — cross-community wikilinks, cohesion scores, audit trail, navigation footer
...
- Fix: `pip install tracely360` → `pip install tracely360` in skill Step 1 (critical install bug)
...
- Docs: README reframed as Claude Code skill; Karpathy problem → tracely360 answer framing
...
- Memory feedback loop - Q&A results saved to `tracely360-out/memory/`, extracted on `--update`
...
- `tracely360 install` CLI - copies skill to `~/.claude/skills/` and registers in `CLAUDE.md`
