...
```
/tracely360                     # full pipeline on current directory
/tracely360 <path>              # full pipeline on specific path
/tracely360 <path> --update     # incremental - re-extract only new/changed files
/tracely360 <path> --no-viz     # skip visualization, just report + JSON
/tracely360 <path> --wiki       # build agent-crawlable wiki
/tracely360 query "<question>"  # BFS traversal - broad context
```
...
### Step 1 - Ensure tracely360 is installed

```python
python -c "import tracely360; import sys; from pathlib import Path; Path('tracely360-out').mkdir(exist_ok=True); Path('tracely360-out/.tracely360_python').write_text(sys.executable)"
```

If the import fails, install first:

```python
python -m pip install tracely360 -q
```
...
Path('tracely360-out/.tracely360_detect.json').write_text(json.dumps(result, indent=2))
...
detect = json.loads(Path('tracely360-out/.tracely360_detect.json').read_text())
code_files = []
...
if code_files:
    ...
    Path('tracely360-out/.tracely360_ast.json').write_text(json.dumps(result, indent=2))
    print(f'AST: {len(result[\"nodes\"])} nodes, {len(result[\"edges\"])} edges')
else:
    Path('tracely360-out/.tracely360_ast.json').write_text(json.dumps({'nodes':[],'edges':[],'input_tokens':0,'output_tokens':0}))
...
python -c "
import json
from tracely360.cache import check_semantic_cache
from pathlib import Path
...
if cached_nodes or cached_edges:
    Path('tracely360-out/.tracely360_cached.json').write_text(json.dumps({'nodes': cached_nodes, 'edges': cached_edges, 'hyperedges': cached_hyperedges}))
Path('tracely360-out/.tracely360_uncached.txt').write_text('\n'.join(uncached))
...
```
You are a tracely360 extraction subagent. Read the files listed and extract a knowledge graph fragment.
...
ast = json.loads(Path('tracely360-out/.tracely360_ast.json').read_text())
all_nodes.extend(ast.get('nodes', []))
all_edges.extend(ast.get('edges', []))

cached_path = Path('tracely360-out/.tracely360_cached.json')
...
Path('tracely360-out/.tracely360_extract.json').write_text(json.dumps(merged, indent=2))
...
Path('tracely360-out/graph.json').write_text(json.dumps(graph_data, indent=2))
Path('tracely360-out/.tracely360_analysis.json').write_text(json.dumps({
...
extraction = json.loads(Path('tracely360-out/.tracely360_extract.json').read_text())
analysis = json.loads(Path('tracely360-out/.tracely360_analysis.json').read_text())
...
Path('tracely360-out/GRAPH_REPORT.md').write_text(report)
...
from tracely360.build import build_from_json
from tracely360.cluster import cluster
from tracely360.export import to_html
from pathlib import Path
...
Read `tracely360-out/GRAPH_REPORT.md` and share the **God Nodes** and **Surprising Connections** sections directly in the chat — do not ask the user to open the file themselves.
