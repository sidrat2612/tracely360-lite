...
def generate(
    ...
    if endpoint_nodes:
        lines += ["", "## API Endpoints"]
        lines.append(f"- {len(endpoint_nodes)} endpoint(s) detected")
        lines.append("")
        lines.append("| Method | Path | Framework | Source |")
        lines.append("|--------|------|-----------|--------|")
        for _nid, data in sorted(endpoint_nodes, key=lambda x: (x[1].get("method", ""), x[1].get("path", ""))):
            method = data.get("method", "?")
            path = data.get("path", "?")
            framework = data.get("framework", "?")
            src = data.get("source_file", "")
            loc = data.get("source_location", "")
            lines.append(f"| {method} | `{path}` | {framework} | {src} {loc} |")
