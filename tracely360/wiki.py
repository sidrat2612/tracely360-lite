...
# Generates an agent-crawlable wiki: index.md + one article per cluster + god node articles
    ...
    for nid in nodes:
        for neighbor in G.neighbors(nid):
            ...
            if ncid is not None and ncid != own_cid:
                counts[labels.get(ncid, f"Cluster {ncid}")] += 1
...
def _community_article(
    ...
    if remaining > 0:
        lines.append(f"- *... and {remaining} more nodes in this cluster*")
    lines.append("")
    ...
    if cross:
        ...
        lines.append("- No strong cross-cluster connections detected")
    lines.append("")
    ...
    lines += ["---", "", "*Part of the tracely360 knowledge wiki. See [[index]] to navigate.*"]
    return "\n".join(lines)
...
def _god_node_article(G: nx.Graph, nid: str, labels: dict[int, str]) -> str:
    ...
    src = d.get("source_file", "")
    cid = d.get("community")
    community_name = labels.get(cid, f"Cluster {cid}") if cid is not None else None
    ...
    if community_name:
        lines += [f"**Cluster:** [[{community_name}]]", ""]
...
def _index_md(
    ...
    lines: list[str] = [
        ...
        "",
        "> Auto-generated by tracely360. Start here — read cluster articles for context, then drill into god nodes for detail.",
        "",
        f"**{total_nodes} nodes · {total_edges} edges · {len(communities)} clusters**",
        "",
        "---",
        "",
        "## Clusters",
...
def to_wiki(
    ...
    - <ClusterName>.md    — one article per cluster
    ...
    labels = community_labels or {cid: f"Cluster {cid}" for cid in communities}
