...
from tracely360_lite.analyze import _node_community_map
...
def to_cypher(G: nx.Graph, output_path: str) -> None:
    lines = ["// Neo4j Cypher import - generated by /tracely360-lite", ""]
...
def to_html(
    ...
    # Endpoint styling
    _ENDPOINT_COLOR = "#e91e63"
    _ENDPOINT_SHAPE = "star"
    ...
    for node_id, data in G.nodes(data=True):
        ...
        shape = "dot"
        if data.get("file_type") == "endpoint":
            color = _ENDPOINT_COLOR
            shape = _ENDPOINT_SHAPE
            font_size = 12  # always show endpoint labels
        vis_nodes.append({
            "id": node_id,
            "label": label,
...
<head>
<meta charset="UTF-8">
<title>tracely360-lite - {title}</title>
...
def to_obsidian(
    ...
    # Map file_type → tracely360-lite tag
    _FTYPE_TAG = {
        "code": "tracely360-lite/code",
        "document": "tracely360-lite/document",
        "paper": "tracely360-lite/paper",
        "image": "tracely360-lite/image",
    }
    ...
    for node_id, data in G.nodes(data=True):
        ...
        ftype_tag = _FTYPE_TAG.get(ftype, f"tracely360-lite/{ftype}" if ftype else "tracely360-lite/document")
        dom_conf = _dominant_confidence(node_id)
        conf_tag = f"tracely360-lite/{dom_conf}"
    ...
    for idx, cid in enumerate(sorted_cids):
        ...
        for m_idx, node_id in enumerate(sorted_members):
            ...
            canvas_nodes.append({
                "id": f"n_{node_id}",
                "type": "file",
                "file": f"tracely360-lite/obsidian/{fname}.md",
                "x": nx_x,
                "y": nx_y,
                "width": 180,
                "height": 60,
