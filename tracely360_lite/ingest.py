...
from tracely360_lite.security import safe_fetch, safe_fetch_text, validate_url
...
def ingest(url: str, target_dir: Path, author: str | None = None, contributor: str | None = None) -> Path:
    """
    Fetch a URL and save it into target_dir as a tracely360-lite-ready file.
    ...
    try:
        ...
        if url_type == "youtube":
            from tracely360_lite.transcribe import download_audio
...
def save_query_result(
    ...
    Files are stored in memory_dir (typically tracely360-lite-out/memory/) with YAML frontmatter
    that tracely360-lite's extractor reads as node metadata. This closes the feedback loop:
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fetch a URL into a tracely360-lite /raw folder")
    ...
    print(f"Ready for tracely360-lite: {out}")
